...
to Australia.  It is a member of the Megapodiidae family, which are notable for 
incubating their eggs in mounds that provide an external source of heat rather
than in nests heated with their own body.  The malleefowl is distinct from other
members of the family in that it builds particularly elaborate mounds and
inhabits arid habitats rather than damp forests.
...
Solar radiation and fermentation of the organic material maintains a constant
temperature of 33°C within the nest, which is necessary for egg development.
Because sand and organic matter are required for nest construction, malleefowl
tend to habitate in arid to semi-arid shrublands and low woodlands (the bird is
named after the mallee tree, which is common in such habitats).  Within the
context of these habitats, malleefowl population concentration tends to be
highest in areas with higher rainfall and more fertile soil.

Although records suggests that the malleefowl was common prior to European
colonization, its population has declined due to predation by introduced
predators (e.g., red fox) and habitat loss and fragmentation due to human
development, grazing by livestock, and wildfires.  Today, the malleefowl is
listed as vulnerable on the IUCN Red List and under the Australian Environment
...
for the malleefowl.  Although the report had insufficient evidence to accurately
assess malleefowl conservation status across Australia, it cited detailed
monitoring of malleefowl populations in Victoria and NSW that showed steep
decline in the decade leading up to the NRP.  Additionally, the fact that many
populations were small and isolated as well as the reality that dynamics driving
malleefowl population decreases were unlikely to abate lead the government to
take action.

The NRP stated several objectives, including reducing habitat loss, grazing
pressure by livestock, fire threat, and predation.  However, these initiatives
are unlikely to be successful if climate change renders current malleefowl
habitats unviable.  Patterns of rainfall and temperature in particular may be
significant, since they could impact breeding success.
...
malleefowl habitat to expand or contract, some objectives may need to be
curtailed or added to the plan (e.g., it would make little sense to reduce
predation in an area that will soon become unviable for malleefowl).
...
Data was cleaned to include only trustworthy, precise, non-duplicative
observations during the years covered by our historical climate data (1970 -
2000) and plotted to verify a lack of outliers.
...
aggregated to a 2.5 acr-minute resolution
...
Data were projected onto the Geoscience Australia Lambert
projection (EPSG: 3112) and combined.
...
Wettest Month).  We selected the following variables for modelling as being
conceptually justified by the malleefowl's lifecycle:  Annual Mean Temperature,
Isothermality, Max Temperature of Warmest Month, Min Temperature of Coldest
Month, Annual Precipitation, Precipitation of Wettest Month, Precipitation of
Driest Month (Bio1, Bio3, Bio5, Bio6, Bio12, Bio13, Bio14).  Including
elevation, sand composition of soil, and carbon density of soil, we considered
10 potential coefficients to model.

```{r aic}
# Select relevant columns from historical and future rasters
bioclim_h <- subset(bioclim_h, c("bio1", "bio3", "bio5", "bio6", "bio12",
  "bio13", "bio14", "elev", "sand_30-60cm", "ocd_30-60cm"))
bioclim_f1 <- subset(bioclim_f1, c("bio1", "bio3", "bio5", "bio6", "bio12",
  "bio13", "bio14", "elev", "sand_30-60cm", "ocd_30-60cm"))
bioclim_f2 <- subset(bioclim_f2, c("bio1", "bio3", "bio5", "bio6", "bio12",
  "bio13", "bio14", "elev", "sand_30-60cm", "ocd_30-60cm"))
```
