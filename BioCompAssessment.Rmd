...
m.e <- m.e[m.e$dup == FALSE, ] # n = 732

# Create an object with only geo coord columns, since this is all that is
# needed for plotting.
m <- m.e[, c(17, 19)]

# Declare geo coords extent, create an SF object for occurrences, and plot observations.
...
Comparing model ten (another model with a low AIC score) to model 9, model ten had lower residual deviance, a measure of model fit.
...
The area under the ROC curve was .98, indicating little trade-off between sensitivity and specificity across threshold of presence.
...
# Plot single scenario maps
ggplot2::ggplot() +
  ggplot2::ggtitle("1970-2000 Malleefowl Distribution") +
  ggplot2::geom_tile(
    data = as.data.frame(glm.map, xy = TRUE),
    aes(x = x, y = y, fill = lyr1)) +
  ggplot2::scale_fill_manual(values = c("ivory", "blue")) +
  ggplot2::theme(
    panel.background = element_rect(fill = "lightblue"),
    panel.ontop = FALSE,
    panel.grid = element_blank(),
...
ggplot2::ggplot() +
  ggplot2::ggtitle("2021-2040 Malleefowl Distribution") +
  ggplot2::geom_tile(
    data = as.data.frame(glm.map.future1, xy = TRUE),
    aes(x = x, y = y, fill = lyr1)) +
  ggplot2::scale_fill_manual(values = c("ivory", "blue")) +
  ggplot2::theme(
    panel.background = element_rect(fill = "lightblue"),
...
ggplot2::ggplot() +
  ggplot2::ggtitle("2041-2060 Malleefowl Distribution") +
  ggplot2::geom_tile(
    data = as.data.frame(glm.map.future2, xy = TRUE),
    aes(x = x, y = y, fill = lyr1)) +
  ggplot2::scale_fill_manual(values = c("ivory", "blue")) +
  ggplot2::theme(
    panel.background = element_rect(fill = "lightblue"),
...
# Create masks and plot overlaid scenario map.
...
Next, a linear model of EDGE score on HWI was fit.
...
As expected, HWI was not a statistically significant predictor of EDGE score.  The p-value of HWI was .46, above an alpha of 5%, causing the null hypothesis of no relationship between HWI and EDGE score to fail to be rejected.

Additionally, the model itself was a poor fit for the data. R-squared was 7%, demonstrating that the model explained only a small percentage of variation. This is evident in plotting.

```{r stats_plot1, error=FALSE, warning=FALSE, message=FALSE}
# Create a scatter plot of the data and add the linear regression.
plot(b.agg$avg.HWI, b.agg$EDGE)
abline(m.b)
```

To validate the applicability of linear regression as a technique to these data, model diagnostic plots were plotted and assessed.
...
Of course, a negative cannot be proven.  But, this approach failed to demonstrate any statistically significant evidence that dispersability as proxied by HWI relates to Megapodiidae extinction risk.


## Summary

Malleefowl are currently listed as a vulnerable species per the IUCN Red List and Australian law.  Although population size 
