...
```
This threshold was `r round(spec.sense, digits = 3)`. Finally, estimated historical and future distributions were plotted.
...
layout(matrix(c(1, 1), nrow = 1), widths = c(2), heights = c(1.5), TRUE)
...
layout(matrix(c(1, 2), nrow = 1), widths = c(2, 2), heights = c(2, 2), TRUE)
plot(m.b, which = 1:2)
```
