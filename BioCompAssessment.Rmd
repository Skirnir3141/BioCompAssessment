...
```{r figurename, echo=FALSE, fig.cap="Malleefowl; image from Macaulay Library at the Cornell Lab (ML82227771)."}
...
```
Overall, data cleaning reduced records from `r prettyNum(nrow(m.full), big.mark = ",", scientific = FALSE)` to `r prettyNum(nrow(m), big.mark = ",", scientific = FALSE)`.
...
These future time periods were chosen in order to evaluate time scale of malleefowl distribution shift.  SSP 2-4.5 was chosen because it represents a "middle ground" scenario and so should provide a conservative estimate of malleefowl distribution changes.
...
Because malleefowl are likely to have preferences based on elevation, sand composition of soil, and fertility of soil (Benshemesh, 2007), the geodata package was used to pull elevation data from the Shuttle Radar Topography Mission and sand composition and carbon density (as a proxy for soil richness) data from the SoilGRIDS database. Sand composition was measured as the percentage of > .05mm particles in fine earth.  Organic carbon density was measured as kg per cubic meter.

Climate and elevation data were pulled at a 2.5 arc-minute resolution.  Soil data were pulled at a 30 arc-second resolution and aggregated to a 2.5 arc-minute resolution.  A 2.5 arc-minute resolution was chosen because it is reasonably fine and should encompass an individual malleefowl territory (at Australian latitudes, 2.5 arc-minutes is between ~ 3.5-4.5km). Radio-tracking demonstrates that malleefowl range from one to several miles within a year (Benshemesh, 2007).
...
Since logistic regression requires both presence and absence data points, a number of background psuedo-absence points equal to the observation count (n = `r prettyNum(nrow(m), big.mark = ",", scientific = FALSE)`) were generated at random using the Dismo package.
...
```{r absence, error=FALSE, out.width="75%", out.height="75%", fig.align='center', warning=FALSE, message=FALSE}
...
best.fit
#kableExtra::kable_styling(knitr::kable(best.fit$Subsets), font_size = 6)
```

Model 9, which includes all variables except for Bio3, had the lowest AIC score. 

To validate, model 10 (another model with a low AIC score) was fit and compared to model 9.
...
m.10 <- glm(
  ...
  subset = kfold != 1)
```
Model 10 had a slightly lower residual deviance (`r round(m.10$deviance, digits = 2)` vs `r round(m.9$deviance, digits = 2)`). But, the difference was very minor and model 10 included an additional coefficient.  AIC reasonably picked model 9 as the most parsimonious model.
...
Other relationships are less intuitive.  For example, contrary to expectations, probability of presence decreases with Bio12  (Annual Precipitation) and carbon density of soil.  But, this may reflect a preference against damp forested habitats overall rather than a preference against these variables within viable malleefowl habitat.
...
```{r roc, error=FALSE, out.width="75%", out.height="75%", fig.align='center', warning=FALSE, message=FALSE}
...
The area under the ROC curve was `r round(slot(glm.eval, "auc"), digits = 3)`, indicating little trade-off between sensitivity and specificity across threshold of presence.
...
```
This threshold was `r round(spec.sense, digits = 3)`. Finally, estimated current distribution was plotted along with future distribution for 2021-2040 and 2041-2060.
...
terra::plot(
  ...
  col = c("grey", "blue"),
  axes = FALSE,
  main = "Overlaid Time Period Distribution",
  font.main = 1,
  cex.main = 1)
...
```
Specifically, malleefowl habitat is expected to decline to `r scales::percent(round(sum(h.f1[h.f1$F1 == "TRUE", ]$Freq) / sum(h.f1[h.f1$Hist == "TRUE", ]$Freq), digits = 3))` of its historical range by 2021-2040 and to `r scales::percent(round(sum(h.f2[h.f2$F2 == "TRUE", ]$Freq) / sum(h.f2[h.f2$Hist == "TRUE", ]$Freq), digits = 3))` by 2041-2060.

In particular, Western Australia is forecast to be uninhabitable by malleefowl between 2040 and 2060.  The largest viable habitat is expected to be in western New South Wales.  However, some new viable habitat is expected to develop in coastal Queensland.
...
However, this relationship may not hold among Megapodiidae species.  While some are strong flyers, many are heavy and poor at flying (Harris, Burks & Leache, 2014). So, HWI may be a poor proxy for dispersability and so a poor predictor of extinction risk.
...
# Output results
knitr::kable(b.agg, digits = c(2, 1))
```

No obvious relationship was evident.  To evaluate, a linear model of EDGE score on HWI was fit.
...
These issues will cause the estimate of model standard error (and therefore p-values and statistical significance) to be biased.

There is likely one more issue as well: because the data are evolutionarily related species and not random observations, the assumption of independence is violated.
...
Finally, this report assessed whether Megapodiidae extinction risk was related to dispersability.  No relationship was expected due to generally poor flying ability by Megapodiidae. Of course, a negative cannot be proven.  But, a linear regression failed to demonstrate any statistically significant evidence that dispersability relates to Megapodiidae extinction risk.
...
GBIF (2023) Global Biodiversity Information Facility. https://www.gbif.org/ [Accessed: 16 November 2023].
